...
async def test_decorators():
    ...
    async def test_distribute_coins(ctx, amount: int):
        ...
        print(
            f"📊 Comando executado com ctx={type(ctx).__name__}, amount={amount} (tipo: {type(amount).__name__})"
        )
    ...
    async def test_clan_pay(ctx, member: discord.Member, amount: int):
        ...
        print(
            f"💰 Comando executado com ctx={type(ctx).__name__}, member={member}, amount={amount} (tipo: {type(amount).__name__})"
        )
    ...
    async def test_create_lottery(ctx, *, args=None):
        """Função de teste que simula create_lottery"""
        print(
            f"🎲 Comando executado com ctx={type(ctx).__name__}, args={args} (tipo: {type(args).__name__})"
        )
