...
class EconomySystem:
    ...
    def __init__(self, bot):
        ...
        # Dados em memória
        self.user_data = {}
        self.voice_tracking = {}  # {user_id: {'start_time': datetime, 'channel_id': int, 'last_reward_time': datetime}}
        self.voice_status_messages = {}  # {channel_id: {'message': Message, 'last_update': datetime}}
    ...
    def load_data(self):
        ...
        try:
            if os.path.exists(self.data_file):
                ...
                logger.info(f"Dados de economia carregados: {len(self.user_data)} usuários")
            else:
                self.user_data = {}
    ...
    async def voice_reward_task(self):
        """Task que roda a cada 2 minutos para dar recompensas graduais de voz"""
        try:
            ...
            for user_id, tracking_data in list(self.voice_tracking.items()):
                ...
                # Calcular tempo desde a última recompensa
                time_since_reward = current_time - last_reward_time
                minutes_since_reward = time_since_reward.total_seconds() / 60
                
                # Dar recompensa a cada 2 minutos completos
                if minutes_since_reward >= 2:
                    # Calcular quantos blocos de 2 minutos se passaram
                    blocks = int(minutes_since_reward // 2)
                    
                    # 1 hora = 20 AC, então 2 minutos = (2/60) * 20 = 0.67 AC
                    # Para manter números inteiros: 3 blocos de 2min = 2 AC (6min = 2AC)
                    coins_per_3_blocks = 2  # 2 AC a cada 6 minutos (3 blocos)
                    total_coins = (blocks // 3) * coins_per_3_blocks
                    
                    # Se temos blocos suficientes para recompensar
                    if total_coins > 0:
                        self.add_coins(user_id, total_coins, f"Tempo em voz ({(blocks // 3) * 6} min)")
                        
                        # Atualizar tempo de voz
                        user_data = self.get_user_data(user_id)
                        user_data['voice_time'] += (blocks // 3) * 6
                        
                        # Atualizar tempo da última recompensa (apenas para blocos recompensados)
                        minutes_rewarded = (blocks // 3) * 6
                        new_last_reward = last_reward_time + timedelta(minutes=minutes_rewarded)
                        self.voice_tracking[user_id]['last_reward_time'] = new_last_reward
                        
                        logger.info(f"Usuário {user_id} recebeu {total_coins} AC por {minutes_rewarded} min em voz")
                ...
                try:
                    ...
                    if channel_id in self.voice_status_messages:
                        ...
                        try:
                            ...
                            self.voice_status_messages[channel_id]['last_update'] = current_time
                    ...
                    if channel_id not in self.voice_status_messages:
                        try:
                            ...
                            logger.warning(f"Sem permissão para enviar mensagem no canal {channel_id}")
                        except Exception as e:
                            logger.error(f"Erro ao enviar mensagem no canal {channel_id}: {e}")
    ...
    async def create_voice_status_embed(self, user_ids: List[int], current_time: datetime) -> discord.Embed:
        ...
        for user_id in user_ids:
            if user_id in self.voice_tracking:
                ...
                # Calcular AC ganho até agora (2 AC a cada 6 minutos)
                coins_earned = (total_minutes // 6) * 2
        ...
        if status_lines:
            ...
            embed.add_field(
                name="📊 Nenhum usuário ativo",
                value="Canal vazio",
                inline=False
            )
        
        embed.set_footer(text="ARCA Bot | Atualizado a cada 2 minutos")
    ...
    async def backup_task(self):
        ...
        try:
            ...
            if os.path.exists(self.data_file):
                ...
                backup_file = os.path.join(self.backup_dir, f"economy_backup_{timestamp}.json")
                shutil.copy2(self.data_file, backup_file)
                
                # Manter apenas os últimos 10 backups
                backups = sorted([f for f in os.listdir(self.backup_dir) if f.startswith('economy_backup_')])
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        if before.channel and self.is_in_comms_category(before.channel):
            if user_id in self.voice_tracking:
                ...
                # Dar recompensa final se houver tempo não recompensado (mínimo 3 minutos)
                if minutes_since_reward >= 3:
                    # Aplicar a mesma lógica: 2 AC a cada 6 minutos
                    final_coins = int(minutes_since_reward / 6) * 2
                    if final_coins > 0:
                        self.add_coins(user_id, final_coins, f"Tempo final em voz ({minutes_since_reward:.1f} min)")
                ...
                logger.info(f"Usuário {user_id} saiu da call após {total_minutes:.1f} minutos")
        
        # Usuário entrou em um canal válido
        if after.channel and self.is_in_comms_category(after.channel):
            self.voice_tracking[user_id] = {
                'start_time': current_time,
                'channel_id': after.channel.id,
                'last_reward_time': current_time
            }
            logger.info(f"Usuário {user_id} entrou na call no canal {after.channel.id}")
