...
class RateLimiter:
    ...
    def add_limit(self, name: str, rate_limit: RateLimit):
        ...
        logger.info(
            f"Rate limit adicionado: {name} - {rate_limit.max_uses}/{rate_limit.window}s"
        )

    def _get_bucket_key(
        self, limit_name: str, user_id: int, guild_id: Optional[int] = None
    ) -> str:
    ...
    def _get_or_create_bucket(self, bucket_key: str) -> RateLimitBucket:
        ...
        if bucket_key not in self.buckets:
            self.buckets[bucket_key] = RateLimitBucket(
                uses=deque(), last_reset=time.time()
            )
    ...
    def get_user_status(
        self, user_id: int, limit_name: str, guild_id: Optional[int] = None
    ) -> Dict:
        ...
        cooldown_remaining = (
            max(0, bucket.limit_until - current_time) if is_limited else 0
        )
...
class CommandRateLimiter:
    ...
    def _setup_default_limits(self):
        ...
        # Comando diário - permitir algumas verificações por minuto
        self.rate_limiter.add_limit(
            "daily",
            RateLimit(
                max_uses=3,  # 3 tentativas por minuto (para verificar status)
                window=60,  # 1 minuto (em vez de 24 horas)
        ...
        self.rate_limiter.add_limit(
            "economy",
            RateLimit(
                max_uses=5, window=60, limit_type=RateLimitType.PER_USER, cooldown=30
            ),
        )
        ...
        self.rate_limiter.add_limit(
            "lottery_buy",
            RateLimit(
                max_uses=10, window=60, limit_type=RateLimitType.PER_USER, cooldown=60
            ),
        )
        ...
        self.rate_limiter.add_limit(
            "admin",
            RateLimit(
                max_uses=20, window=60, limit_type=RateLimitType.PER_USER, cooldown=120
            ),
        )
        ...
        self.rate_limiter.add_limit(
            "general",
            RateLimit(
                max_uses=30, window=60, limit_type=RateLimitType.PER_USER, cooldown=60
            ),
        )
    ...
    async def check_command_rate_limit(self, ctx, limit_name: str = "general") -> bool:
        ...
        if not allowed:
            ...
            if cooldown and cooldown > 0:
                ...
                await ctx.send(
                    "⏳ **Rate Limit Atingido**\n"
                    "Você atingiu o limite de uso para este comando.\n"
    ...
    def get_user_limits_status(
        ...
        for limit_name in self.rate_limiter.limits.keys():
            status[limit_name] = self.rate_limiter.get_user_status(
                user_id, limit_name, guild_id
            )
        return status
