...
class WalletPanel:
    ...
    def __init__(self, bot, economy_system):
        ...
        self.panel_data_file = "data/panel_data.json"  # Arquivo para salvar dados do painel
        ...
        try:
            if os.path.exists(self.panel_data_file):
                with open(self.panel_data_file, 'r', encoding='utf-8') as f:
                    ...
                    logger.info(f"📂 Dados do painel carregados: {len(data)} servidores")
            else:
                logger.info("📂 Nenhum arquivo de dados do painel encontrado, será criado automaticamente")
    ...
    async def _find_existing_panels(self):
                        ...
                        try:
                            ...
                            if channel:
                                ...
                                if message and message.author == self.bot.user:
                                    ...
                                    panels_found += 1
                                    logger.info(f"✅ Painel salvo recuperado no servidor {guild.name} (ID: {message.id})")
                            ...
                            logger.warning(f"⚠️ Painel salvo não pôde ser recuperado no servidor {guild.name}: {e}")
        except Exception as e:
        ...
        for guild in self.bot.guilds:
            ...
            try:
                ...
                if not channel:
                    logger.warning(f"❌ Canal '{self.config.wallet_panel_channel}' não encontrado no servidor {guild.name}")
                    continue
                
                logger.info(f"🔎 Verificando canal #{channel.name} no servidor {guild.name}...")
                ...
                async for message in channel.history(limit=50):
                    if (message.author == self.bot.user and 
                        message.embeds and 
                        "💰 Painel de Carteiras ARCA" in message.embeds[0].title):
                        
                        self.panel_messages[guild.id] = message
                        panels_found += 1
                        panel_found = True
                        logger.info(f"✅ Painel existente encontrado no servidor {guild.name} (ID: {message.id})")
                ...
                if not panel_found:
                    ...
                    logger.info(f"❓ Nenhum painel encontrado no servidor {guild.name}, criando novo...")
                    success = await self.create_panel_in_guild(guild)
                    if success:
                        panels_created += 1
                        logger.info(f"✅ Novo painel criado no servidor {guild.name}")
                    else:
                        logger.error(f"❌ Falha ao criar painel no servidor {guild.name}")
        ...
        logger.info(f"📊 Resumo de painéis: {panels_found} encontrados, {panels_created} criados, {total_guilds} servidores verificados")
    ...
    async def _update_loop(self):
        ...
        logger.info(f"🔄 Loop de atualização automática iniciado (intervalo: {self.update_interval//60} minutos)")
        
        while self.is_running:
            try:
                logger.info("🔄 Iniciando atualização automática de todos os painéis...")
                await self._update_all_panels()
                logger.info(f"✅ Atualização automática concluída. Próxima em {self.update_interval//60} minutos")
        ...
        for guild in self.bot.guilds:
            try:
                if guild.id in self.panel_messages:
                    ...
                    logger.warning(f"⚠️ Painel não encontrado para o servidor {guild.name}, tentando criar...")
                    success = await self.create_panel_in_guild(guild)
                    if success:
                        updated_count += 1
                        logger.info(f"✅ Painel criado e atualizado no servidor {guild.name}")
            except Exception as e:
                logger.error(f"❌ Erro ao atualizar painel do servidor {guild.name}: {e}")
        
        logger.info(f"📊 Painéis atualizados: {updated_count}/{len(self.bot.guilds)} servidores")
    
    async def get_wallet_channel(self, guild: discord.Guild) -> Optional[discord.TextChannel]:
    ...
    def _get_wallet_data(self, guild: discord.Guild) -> List[WalletEntry]:
        ...
        for user_id_str, user_data in self.economy.user_data.items():
            ...
            entry = WalletEntry(
                ...
                last_activity=datetime.fromisoformat(user_data.get('last_daily')) if user_data.get('last_daily') else datetime.now()
            )
    ...
    def _create_panel_embed(self, guild: discord.Guild, wallet_data: List[WalletEntry]) -> discord.Embed:
        ...
        if not wallet_data:
            embed.add_field(
                name="📊 Status",
                value="Nenhum usuário com AC encontrado",
                inline=False
            )
            return embed
        ...
        if len(wallet_data) >= 5:
            ...
            sorted_balances = sorted([entry.balance for entry in wallet_data if entry.balance > 0], reverse=True)
            if sorted_balances:
        ...
        if recent_active:
            ...
            for entry in recent_active:
                days_ago = (datetime.now() - entry.last_activity).days
                if days_ago == 0:
                    activity_str = "hoje"
                elif days_ago == 1:
                    activity_str = "ontem"
    ...
    async def update_panel(self, guild: discord.Guild):
        ...
        try:
            ...
            if not channel:
                ...
                logger.warning(f"⚠️ Canal de painel não encontrado no servidor {guild.name}")
                return
            ...
            if guild.id in self.panel_messages:
                try:
                    ...
                    logger.info(f"🔄 Painel atualizado no servidor {guild.name} (#{channel.name})")
                    return
                except (discord.NotFound, discord.HTTPException):
                    # Mensagem foi deletada, criar nova
                    del self.panel_messages[guild.id]
                    logger.warning(f"⚠️ Mensagem de painel perdida no servidor {guild.name}, criando nova...")
            
            # Criar nova mensagem
            try:
                # Limpar mensagens antigas do bot no canal (últimas 10)
                async for msg in channel.history(limit=10):
                    if msg.author == self.bot.user and "Painel de Carteiras" in (msg.embeds[0].title if msg.embeds else ""):
                        try:
                            await msg.delete()
                        except:
                            pass
                ...
                logger.info(f"✅ Novo painel criado no servidor {guild.name} (#{channel.name})")
