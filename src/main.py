...
try:
    ...
    from src.core.utils.rate_limiter import rate_limiter, rate_limit
    from src.core.utils.permissions import PermissionManager, require_economy_admin, require_lottery_admin, require_admin, handle_permission_error
...
class ARCABot(commands.Bot):
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        try:
            ...
            if not log_channel:
                logger.warning(f'Canal "{self.config.general.log_channel_name}" não encontrado no servidor {after.guild.name}')
                return
    ...
    async def on_command_error(self, ctx, error):
        ...
        logger.error(f'Erro no comando {ctx.command}: {type(error).__name__}: {error}', exc_info=True)
        
        try:
            await ctx.send(f'❌ **Erro inesperado**: {type(error).__name__}\n💡 Contate um administrador se o problema persistir.')
        except:
