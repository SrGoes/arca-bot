...
def setup_economy_commands(bot):
    ...
    async def check_balance(ctx):
        ...
        logger.info(f"Comando !saldo executado por {ctx.author} ({ctx.author.id}) no servidor {ctx.guild.name if ctx.guild else 'DM'}")
        ...
        if cached_data:
            ...
            ctx.bot.cache_manager.set(f"user_balance_{ctx.author.id}", (balance, user_data), 60)
    ...
    async def daily_reward(ctx):
        ...
        logger.info(f"Comando !diario executado por {ctx.author} ({ctx.author.id}) no servidor {ctx.guild.name if ctx.guild else 'DM'}")
        ...
        logger.info(f"Recompensa diária de {reward} AC concedida para {ctx.author} ({ctx.author.id})")
    ...
    async def distribute_coins(ctx, amount: int):
        ...
        logger.info(f"Comando !distribuir executado por {ctx.author} ({ctx.author.id}) com valor {amount}")
        ...
        logger.info(f"Distribuição de {amount} AC para {len(members_in_voice)} membros no canal {voice_channel.name} - Total: {total_distributed} AC")
    ...
    async def clan_pay(ctx, member: discord.Member, amount: int):
        ...
        logger.info(f"Comando !pagar executado por {ctx.author} ({ctx.author.id}) para {member} ({member.id}) com valor {amount}")
        ...
        logger.info(f"Pagamento de {amount} AC realizado para {member} ({member.id}) por {ctx.author} ({ctx.author.id})")
    ...
    @require_economy_admin()
    @rate_limit('admin')
    async def remove_coins(ctx, member: discord.Member, amount: int, *, reason: str = "Remoção administrativa"):
        """Remove moedas de um membro (só admins de economia)"""
        logger.info(f"Comando !remover executado por {ctx.author} ({ctx.author.id}) para {member} ({member.id}) com valor {amount} - Motivo: {reason}")
        ...
        if ctx.bot.economy.remove_coins(member.id, amount):
            ...
            logger.info(f"Remoção de {amount} AC realizada para {member} ({member.id}) por {ctx.author} ({ctx.author.id}) - Motivo: {reason}")
