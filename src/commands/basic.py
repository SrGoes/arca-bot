...
def setup_basic_commands(bot):
    ...
    async def ping(ctx):
        ...
        logger.info(f"Comando !ping executado por {ctx.author} ({ctx.author.id}) no servidor {ctx.guild.name if ctx.guild else 'DM'}")
    ...
    async def info(ctx):
        ...
        logger.info(f"Comando !info executado por {ctx.author} ({ctx.author.id}) no servidor {ctx.guild.name if ctx.guild else 'DM'}")
    ...
    async def help_command(ctx):
        ...
        logger.info(f"Comando !help executado por {ctx.author} ({ctx.author.id}) no servidor {ctx.guild.name if ctx.guild else 'DM'}")
    ...
    async def manage_panel(ctx, action: str = None):
        ...
        if action.lower() == 'criar':
            ...
            if success:
                await ctx.send(f"✅ Painel criado no canal **#{ctx.bot.config.general.wallet_panel_channel}**!")
            else:
                await ctx.send(f"❌ Erro ao criar painel. Verifique se o canal **#{ctx.bot.config.general.wallet_panel_channel}** existe!")
            ...
            embed = discord.Embed(
                title="📊 Status do Sistema de Painel",
            ...
            embed = discord.Embed(
                title="⚙️ Configuração do Painel",
                color=discord.Color.blue()
            )
            embed.add_field(
            ...
            if panel_channel:
                embed.add_field(
                    name="🔒 Permissões",
                    value="✅ OK" if panel_channel.permissions_for(ctx.guild.me).send_messages else "❌ Sem permissão",
                    inline=True
                )
            ...
            await ctx.send("❌ Ação inválida! Use: `criar`, `atualizar`, `status` ou `config`")
    ...
    async def cache_management(ctx, action: str = None):
        ...
        if not action:
            ...
            if stats.get('enabled'):
                embed.add_field(name="📈 Taxa de Acerto", value=f"{stats['hit_rate']}%", inline=True)
                embed.add_field(name="🎯 Acertos", value=stats['hits'], inline=True)
                embed.add_field(name="❌ Erros", value=stats['misses'], inline=True)
                embed.add_field(name="📦 Itens", value=f"{stats['size']}/{stats['max_size']}", inline=True)
                embed.add_field(name="⏱️ TTL Padrão", value=f"{stats['default_ttl']}s", inline=True)
                embed.add_field(name="💡 Comandos", value=f"`{ctx.prefix}cache limpar` - Limpar cache", inline=False)
            else:
                embed.add_field(name="Status", value="❌ Cache desabilitado", inline=False)
    ...
    async def shutdown(ctx):
        ...
        logger.info(f"Comando !desligar executado por {ctx.author} ({ctx.author.id}) no servidor {ctx.guild.name if ctx.guild else 'DM'}")
        ...
        try:
            ...
            # Log da ação
            logger.info(f"Bot sendo desligado por {ctx.author} ({ctx.author.id}) em {ctx.guild.name if ctx.guild else 'DM'}")
